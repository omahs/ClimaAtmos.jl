...
"""
    ...
    of other information). `AtmosSimulation` is mutable, and the state (and the other
    variables) are updated as the simulation moves forward. `AtmosSimulation` provides
    convenient methods to obtain useful information, such as `.state` to retrieve the state.

"""
mutable struct AtmosSimulation{AM, OD}
    atmos_model::AM
    # The integrator object contains the state and the callbacks among the other things
    integrator::OD
end
...
function AtmosSimulation(
    ...
    return AtmosSimulation(atmos_model, integrator)
end
